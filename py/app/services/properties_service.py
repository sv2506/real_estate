...
def get_property(property_id: str) -> PropertySummary | None:
...
def _estimate_monthly_costs(
    ...
    total_range = BriefRange(
        low=fixed_total + variable_low_total,
        high=fixed_total + variable_high_total,
    )

    return breakdown_single, fixed, variable, total_range, assumptions


def get_property_brief(property_id: str) -> PropertyBrief | None:
    ...
    if prop.sqft >= 1600:
        conflicts.append(
            BriefConflict(
                field="Living area",
                values=[
                    f"{prop.sqft} sqft (listing)",
                    f"{max(0, prop.sqft - 80)} sqft (public record)",
                ],
